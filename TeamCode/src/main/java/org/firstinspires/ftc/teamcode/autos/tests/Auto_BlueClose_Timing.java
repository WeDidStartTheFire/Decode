...
@Autonomous(name = "🟦Blue🟦 Close Timing", group = "Test", preselectTeleOp = BLUE_TELEOP_NAME)
public class Auto_BlueClose_Timing extends OpMode {
    private Robot robot;
    ...
    public void stop() {
        ...
        robot.drivetrain.follower.breakFollowing();
        pose = robot.drivetrain.follower.getPose();
        for (int i = 0; i < times.size(); i++) {tm.print("Time"+i, times.get(i));}
        tm.update();

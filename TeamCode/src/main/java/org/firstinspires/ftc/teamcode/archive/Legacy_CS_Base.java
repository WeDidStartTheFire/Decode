...
/*
 * WARNING:
 * This file is old and was used for the Center Stage Season 2023-2024.
 * This file was modified to be compatable with the current FtcRobotController version, meaning that
 * all tfod methods and usages are removed.
 */

/** Centerstage base class that contains common methods and other configuration.
...
public abstract class Legacy_CS_Base extends LinearOpMode {
    ...
    public void setup(color teamColor, boolean useCam) {
        ...
        try {
            ...
            except("carWashMotor (liftMotor) not conected");
        }
        try {
    ...
    public void drive(double inches, dir direction) {
        // double startAngle = imu.getRobotOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX,
        // AngleUnit.DEGREES).firstAngle;
        int checks =
                1; // Number of times the robot will check its orientation during a single drive
                   // movement and correct itself
        if (inches != 0) {
            for (int i = 0; i < checks; i++) {
                encoderDrive(inches / checks, direction);
                // turn(startAngle - imu.getRobotOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX,
                // AngleUnit.DEGREES).firstAngle);
            }
            stopRobot();
        } else {
